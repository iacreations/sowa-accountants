...
from .models import (
    Expense, ExpenseCategoryLine, ExpenseItemLine, ColumnPreference,
    Bill, BillCategoryLine, BillItemLine,
    PurchaseOrder, PurchaseOrderLine,
    Cheque, ChequeCategoryLine, ChequeItemLine,
    SupplierCredit, SupplierCreditLine,
    PayDownCredit, CreditCardCredit,
    CreditCardCreditCategoryLine, CreditCardCreditItemLine,
    ChequeBillLine, ChequeOpenBalanceLine, SupplierRefund
)
from tenancy.permissions import module_required,company_required
...
def generate_unique_po_no(company, prefix="PO"):
    """
...
def _bill_balance(bill: "Bill", exclude_cheque_id: int | None = None) -> Decimal:
    ...
    Tenant-safe because ChequeBillLine is constrained via bill -> company.
    """
...
def _account_credit_balance(account: "Account", company=None) -> Decimal:
    """
    Returns credit balance for an account:
      opening_balance + SUM(credit - debit)

    When company is provided, only that tenant's journal lines are counted.
...
def _supplier_open_balance_amount(supplier: "Newsupplier", company=None) -> Decimal:
    """
    ...
    supplier_ap_bal = _account_credit_balance(supplier_acc, company=company)  # credit balance in A/P
...
def _supplier_prepayment_balance(supplier, company=None) -> Decimal:
    if not supplier:
        return Decimal("0.00")
...
# post bill to ledger
def _get_or_create_ap_control_account(company=None) -> "Account":
    """
...
def _get_or_create_supplier_ap_subaccount(supplier: Newsupplier, company=None) -> Account:
    """
...
def _post_cheque_to_ledger(cheq: "Cheque"):
    ...
    if cheq.payee_supplier_id and supplier_ap_debit > 0:
        supplier_acc = _get_or_create_supplier_ap_subaccount(cheq.payee_supplier, company=company)
...
def ap_aging_summary(request):
    ...
    bills = _bills_with_outstanding_qs(company).only(
    ...
    vendors = _vendors_qs(company=company)
...
def ap_aging_detail(request):
    ...
    bills = (
        _bills_with_outstanding_qs(company)
        .only("id", "supplier_id", "supplier_name", "bill_no", "bill_date", "due_date", "total_amount")
...
def vendor_balances_report(request):
    ...
    bills = _bills_with_outstanding_qs(company).only(
        "id", "supplier_id", "supplier_name", "bill_date", "due_date", "total_amount"
    )
...
def expenses(request):
    ...
    # Column prefs as before, but tenant-safe
...
def add_expense(request):
        ...
        try:
            with transaction.atomic():
                ...
                if location_id and location_id != "add_new":
                    location_fk = InventoryLocation.objects.for_company(company).filter(pk=location_id).first()
    ...
    context = {
        ...
        "expense_accounts": expense_accounts_qs_,
        "products": Product.objects.for_company(company).all().order_by("name"),
        "customers": Newcustomer.objects.for_company(company).all().order_by("customer_name"),
...
def expense_edit(request, pk: int):
    ...
    exp = get_object_or_404(
        ...
        .select_related("payee_supplier", "payment_account", "location")
        .prefetch_related("cat_lines__category", "item_lines__product"),
        pk=pk
    )
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                supplier_id = request.POST.get("payee_supplier") or ""
                exp.payee_supplier = Newsupplier.objects.for_company(company).filter(pk=supplier_id).first() if supplier_id else None
                ...
                location_id = request.POST.get("location") or ""
                if location_id and location_id != "add_new":
                    exp.location = InventoryLocation.objects.for_company(company).filter(pk=location_id).first()
                else:
                    exp.location = None
...
def get_default_location(company):
    ...
    if not loc:
        ...
        if hasattr(InventoryLocation, "company_id"):
...
def add_bill(request):
    ...
    context = {
        ...
        "all_accounts": all_accounts.order_by("account_name"),
        "products": Product.objects.for_company(company).all().order_by("name"),
        "customers": Newcustomer.objects.for_company(company).all().order_by("customer_name"),
        "suppliers": Newsupplier.objects.for_company(company).all().order_by("company_name"),
        "classes": Pclass.objects.for_company(company).all().order_by("class_name"),
        ...
        "generated_bill_no": generate_unique_bill_no(company),
    }
...
def edit_bill(request, pk: int):
    ...
    context = {
        ...
        # keep your existing bill no usage (template uses generated_bill_no)
...
def bill_detail(request, pk):
    company = request.company
    bill = get_object_or_404(
        Bill.objects.for_company(company).select_related("supplier").prefetch_related(
            Prefetch(
                "category_lines",
...
def generate_unique_cheque_no(company):
    # Simple sequential fallback: last cheque id + 1, padded, company-safe
    last = Cheque.objects.for_company(company).order_by("-id").first()
...
def outstanding_bills_api(request):
    ...
    applied_map = dict(
        ...
        .filter(bill__company=company, bill__supplier_id=supplier_id)
        .values("bill_id").annotate(s=Sum("amount_applied"))
        .values_list("bill_id", "s")
    )

    for b in Bill.objects.for_company(company).filter(supplier_id=supplier_id).order_by("-bill_date"):
    ...
    open_balance = _supplier_open_balance_amount(supplier, company=company)
...
def add_cheque(request):
    ...
    context = {
        "cheque": None,
        "today": timezone.localdate(),
        "generated_cheque_no": generate_unique_cheque_no(company),
        "suppliers": Newsupplier.objects.for_company(company).all().order_by("company_name"),
        "customers": Newcustomer.objects.for_company(company).all().order_by("customer_name"),
        "classes": Pclass.objects.for_company(company).all().order_by("class_name"),
        "products": Product.objects.for_company(company).all().order_by("name"),
        "expense_accounts": expense_qs,
        "bank_accounts": bank_accounts.order_by("account_name"),
...
def cheque_edit(request, pk: int):
    ...
    if cheque.payee_supplier_id:
        ...
        applied_map = dict(
            ChequeBillLine.objects
            .filter(bill__company=company, bill__supplier_id=cheque.payee_supplier_id)
            .values("bill_id").annotate(s=Sum("amount_applied"))
            .values_list("bill_id", "s")
        )

        for b in Bill.objects.for_company(company).filter(supplier_id=cheque.payee_supplier_id).order_by("-bill_date"):
            total = Decimal(str(b.total_amount or "0"))
        ...
        open_balance_value = _supplier_open_balance_amount(cheque.payee_supplier, company=company)
...
def purchase_order(request):
    ...
    context = {
        ...
        "generated_po_number": generate_unique_po_no(company),
        "today": timezone.localdate(),
        "po": None,
        "item_lines": [],
    }
...
def purchase_order_detail(request, pk: int):
    company = request.company
    po = get_object_or_404(
        PurchaseOrder.objects.for_company(company).prefetch_related(
...
def supplier_credit_edit(request, pk: int):
    ...
    credit = get_object_or_404(
        SupplierCredit.objects.for_company(company).select_related("supplier")
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                location_id = request.POST.get("location") or ""
                credit.location = InventoryLocation.objects.for_company(company).filter(pk=location_id).first() if location_id else None
                credit.memo = request.POST.get("memo") or ""

                if new_ref_no and new_ref_no != (credit.ref_no or ""):
                    if SupplierCredit.objects.for_company(company).exclude(pk=credit.pk).filter(ref_no=new_ref_no).exists():
...
def supplier_credit_detail(request, pk: int):
    company = request.company
    credit = get_object_or_404(
        SupplierCredit.objects.for_company(company).select_related("supplier").prefetch_related(
            Prefetch(
                "lines",
...
def add_paydown_credit(request):
    ...
    if hasattr(Account, "company_id"):
        credit_cards_qs = credit_cards_qs.filter(company=company)
    credit_cards_qs = credit_cards_qs.order_by("account_name")

    bank_accounts_qs = deposit_accounts_qs()
    if hasattr(bank_accounts_qs, "for_company"):
        bank_accounts_qs = bank_accounts_qs.for_company(company)
    elif hasattr(bank_accounts_qs.model, "company_id"):
        bank_accounts_qs = bank_accounts_qs.filter(company=company)
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                supplier = (
                    Newsupplier.objects.for_company(company).filter(pk=supplier_id).first()
...
def paydown_credit_edit(request, pk: int):
    ...
    pdc = get_object_or_404(
        PayDownCredit.objects.for_company(company).select_related(
            "credit_card", "bank_account", "payee_supplier", "location"
        ),
        pk=pk,
    )
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                location_id = request.POST.get("location") or ""
                pdc.location = InventoryLocation.objects.for_company(company).filter(pk=location_id).first() if location_id else None
...
def paydown_credit_list(request):
    ...
    qs = (
        ...
        .select_related("credit_card", "bank_account", "payee_supplier", "location")
        .order_by("-payment_date", "-id")
    )
...
def add_credit_card_credit(request):
    ...
    if hasattr(deposit_accounts, "for_company"):
        deposit_accounts = deposit_accounts.for_company(company)
    elif hasattr(deposit_accounts.model, "company_id"):
        deposit_accounts = deposit_accounts.filter(company=company)
    ...
    expense_accounts = expense_accounts.order_by("account_name")

    products = Product.objects.for_company(company).all().order_by("id")
    suppliers = Newsupplier.objects.for_company(company).all().order_by("company_name")
    customers = Newcustomer.objects.for_company(company).all().order_by("customer_name")
    classes = Pclass.objects.for_company(company).all().order_by("class_name") if "Pclass" in globals() else []
    locations = InventoryLocation.objects.for_company(company).all().order_by("name")
        ...
        try:
            with transaction.atomic():
                ...
                for idx, cat_id in enumerate(cat_categories):
                    ...
                    customer = (
                        Newcustomer.objects.for_company(company).filter(pk=cust_id).first()
                        if cust_id
                        else None
                    )
                    pclass = (
                        Pclass.objects.for_company(company).filter(pk=cls_id).first()
                        if cls_id
                        else None
                    )
...
def credit_card_credit_edit(request, pk: int):
    ...
    cc = get_object_or_404(
        CreditCardCredit.objects.for_company(company).select_related("credit_card", "payee_supplier", "location"),
        pk=pk,
    )
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                location_id = request.POST.get("location") or ""
                cc.location = InventoryLocation.objects.for_company(company).filter(pk=location_id).first() if location_id else None
...
def credit_card_credit_detail(request, pk: int):
    ...
    cc = get_object_or_404(
        ...
        .select_related("credit_card", "payee_supplier", "location")
        .prefetch_related("category_lines__category", "item_lines__product"),
        pk=pk,
    )
...
def supplier_prepayments_list(request):
    company = request.company
    suppliers = Newsupplier.objects.for_company(company).order_by("company_name")

    rows = []
    for s in suppliers:
        bal = _supplier_prepayment_balance(s, company=company)
...
def supplier_refund_new(request, supplier_id: int):
    company = request.company
    supplier = get_object_or_404(Newsupplier.objects.for_company(company), pk=supplier_id)
    accounts = deposit_accounts_qs()  # Bank/Cash list
    if hasattr(accounts, "for_company"):
        accounts = accounts.for_company(company)
    elif hasattr(accounts.model, "company_id"):
        accounts = accounts.filter(company=company)

    max_refundable = _supplier_prepayment_balance(supplier, company=company)
